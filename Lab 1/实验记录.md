...
2. worker 执行 Map 或 Reduce 任务时，都先将输出写入临时文件，然后再重命名；
3. 由 worker 执行 Reduce 任务中的排序工作，减轻 coordinator 负担；
4. worker 完成任务后，向 coordinator 报告；
5. coordinator 根据目前的任务状态，决定分派 Map、Reduce 还是 Done 任务，Done 表示整个 MapReduce 任务已经完成；
6. 只有 Map 任务全部完成后才能分派 Reduce 任务，因此当 Map 任务都在执行时，coordinator 不回复 worker 的请求；
7. 当 Reduce 任务都在执行时，coordinator 也不回复 worker 的请求；
8. 当所有 Reduce 任务完成后，且 worker 又向 coordinator 请求任务时，分派 Done 任务让 worker 自行退出，这里不管 worker 是否正确退出；   
...
执行 Reduce 任务：根据任务编号读取中间文件，调用 `reducef`，写入临时文件，重命名为最终文件。
